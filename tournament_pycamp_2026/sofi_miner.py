...
class BotLogic:
    ...
    def go_to_position(self, postion_to_go, state, change_power=True, exploration_mode=True):
        ...
        if postion_to_go:
            position_to_go = min(possible_fly_positions, key=lambda p: p.distance_to(postion_to_go))
        else:
            position_to_go = random.choice(possible_fly_positions)

        if not position_to_go:
            return FLY_TO, state.position


        return FLY_TO, position_to_go
    ...
    def go_to_mine(self, state, change_power=True):
        ...
        return self.go_to_position(None, state, change_power)
