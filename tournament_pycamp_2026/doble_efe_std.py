...
class BotLogic:
    ...
    def turn(
        ...
        enemy_positions = {pos for pos, thing in radar_contacts.items() if thing == SPACESHIP}
        log("=========== enemies", enemy_positions)
        if not enemy_positions:
            return self._turn_alone(turn_info)

        if all(pos in self.home_base_positions for pos in enemy_positions):
            # all enemies in the base, move freely as they can't shoot
            return self._turn_alone(turn_info)

        distance_from_center = turn_info.position.distance_to(HOME_BASE_POSITION)
        if distance_from_center < 4:
            # don't stuck in the base anyway
            return self._turn_alone(turn_info)

        if random.random() > 0.5:
            # move freely half of the times that enemies are in sight anyway
            return self._turn_alone(turn_info)

        for enemy_pos in enemy_positions:
            if position.distance_to(enemy_pos) <= ATTACK_RADIUS:
                return self._turn_ship_in_range(turn_info)

        return self._turn_ship_far_away(turn_info)
    ...
    def _turn_alone(self, turn_info):
        ...
        # not returning? only chase for asteroids if moved away from the base
        distance_from_center = turn_info.position.distance_to(HOME_BASE_POSITION)
        ...
        if all_asteroid_positions and distance_from_center > 4:
            distances = [
    ...
    def _turn_ship_in_range(self, turn_info):
        """Move after seeing a ship that we can shoot."""
        log("============== turn, ship in range")
        _total_money = sum(turn_info.leader_board.values())
        if self.mode == AGGRESIVE and _total_money > 1500:
            if random.random() < 0.8:
                desired = {ENGINES: 0, SHIELDS: 0, LASERS: 3}
            else:
                desired = {ENGINES: 1, SHIELDS: 0, LASERS: 2}
            return POWER_TO, desired

        return self._escape_to_base(turn_info)

    def _turn_ship_far_away(self, turn_info):
        """Move after seeing a ship very far away."""
        log("============== turn, ship far away")
        if self.mode == AGGRESIVE:
            if random.random() < 0.8:
                desired = {ENGINES: 0, SHIELDS: 3, LASERS: 0}
            else:
                desired = {ENGINES: 1, SHIELDS: 2, LASERS: 0}
            return POWER_TO, desired
